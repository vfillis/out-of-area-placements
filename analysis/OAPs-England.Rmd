...
```{r}
OAPs.receivers.oct20$NHS <- grepl("^[R]|^[T]", OAPs.receivers.oct20$Breakdown1Code)
```
...
```{r}
OAPs.receivers.oct20 <- OAPs.receivers.oct20[!(OAPs.receivers.oct20$Breakdown1Code=="999" | OAPs.receivers.oct20$Breakdown1Code=="England"),]
```

### Remove those with 0 days

```{r}
OAPs.receivers.oct20 <- OAPs.receivers.oct20[!(OAPs.receivers.oct20$value=="0"),]
```
...
sum(table(OAPs.receivers.oct20$NHS))

# How many are NHS (= TRUE) and how many private (= FALSE)?
table(OAPs.receivers.oct20$NHS)
