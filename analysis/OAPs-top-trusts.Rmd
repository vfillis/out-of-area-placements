---
title: "Out of Area Placements Top Trusts"
author: "Vanessa Fillis"
date: "08/01/2021"
output: html_document
---

# Out of Area Placements in Mental Health Services in NHS trusts in England
...
## Filter for total inappropriate OAP days in trusts for each period
...
I want to know which trusts have had the most inappropriate OAP days in the latest available time period: October 2020. 

```{r}
OAPs.trusts.days.oct20 <- subset(OAPs.trusts.days, OAPs.trusts.days$beginning.of.publication.period == "2020-10-01")
```

### Top 5 of October 2020

Look at the top 10 trusts with most inappropriate OAP days in October 2020. Since the total number of England is also included, I will consider the first 6 rows instead of the first five. 

```{r}
OAPs.trusts.days.oct20 <- arrange(OAPs.trusts.days.oct20, desc(value))

#Which ones are the top 5?
OAPs.trusts.days.oct20.topfive <- head(OAPs.trusts.days.oct20, 6)
```

### Export

```{r}
write.csv(OAPs.trusts.days.oct20.topfive, file = "OAPs_trusts_days_top5oct20.csv")
```

## Filter for costs caused by inappropriate OAPs in trusts for each period
...
I want to know which trusts have had the highest costs caused by inappropriate OAPs in the latest available time period: October 2020. 

```{r}
OAPs.trusts.costs.oct20 <- subset(OAPs.trusts.costs, OAPs.trusts.costs$beginning.of.publication.period == "2020-10-01")
```

### Top 5 of October 2020

Look at the top 5 trusts with most inappropriate OAP days in October 2020. 

```{r}
OAPs.trusts.costs.oct20 <- arrange(OAPs.trusts.costs.oct20, desc(value))

#Which ones are the top 5?
OAPs.trusts.costs.oct20.topfive <- head(OAPs.trusts.costs.oct20, 6)
```

### Export

```{r}
write.csv(OAPs.trusts.costs.oct20.topfive, file = "OAPs_trusts_costs_top5oct20.csv")
```
